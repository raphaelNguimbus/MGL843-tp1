...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Generate TP2 metric visualizations from a CSV file."
    )
    parser.add_argument(
        "csv",
        help="Path to input CSV file (must contain ClassName, SLOC, WMC, CBO_in, CBO_out, RFC, TCC).",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--prefix",
        default="fig-tp2",
        help="Filename prefix for output images (default: fig-tp2).",
    )
    return parser
...
def main():
    ...
    plt.tight_layout()
    p = out_path(args.output, f"{args.prefix}-sloc.png")
    plt.savefig(p, dpi=300, bbox_inches="tight")
    print(f"Visualisation sauvegardée: {p}")
    plt.close()
    ...
    ax.bar_label(bars, fontweight="bold", fontsize=10, padding=4)
    plt.tight_layout()
    p = out_path(args.output, f"{args.prefix}-wmc.png")
    plt.savefig(p, dpi=300, bbox_inches="tight")
    print(f"Visualisation sauvegardée: {p}")
    plt.close()
    ...
    plt.tight_layout()
    p = out_path(args.output, f"{args.prefix}-cbo.png")
    plt.savefig(p, dpi=300, bbox_inches="tight")
    print(f"Visualisation sauvegardée: {p}")
    plt.close()
    ...
    ax.bar_label(bars, fontweight="bold", fontsize=10, padding=4)
    plt.tight_layout()
    p = out_path(args.output, f"{args.prefix}-rfc.png")
    plt.savefig(p, dpi=300, bbox_inches="tight")
    print(f"Visualisation sauvegardée: {p}")
    plt.close()
    ...
    plt.tight_layout()
    p = out_path(args.output, f"{args.prefix}-scatter.png")
    plt.savefig(p, dpi=300, bbox_inches="tight")
    print(f"Visualisation sauvegardée: {p}")
    plt.close()
    ...
    plt.tight_layout()
    p = out_path(args.output, f"{args.prefix}-tcc.png")
    plt.savefig(p, dpi=300, bbox_inches="tight")
    print(f"Visualisation sauvegardée: {p}")
    plt.close()
